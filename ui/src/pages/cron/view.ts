  ...
  return html`
    <details class="cron-advanced">
      ...
      <div class="cron-rows">
        ...
        ${renderRow({
          label: "Session key",
          help: "Optional routing key for job delivery and wake routing.",
          control: html`
            <input
              id="cron-session-key"
        ...
        ${ctx.isAgentTurn
          ? html`
              ${renderRow({
                label: "Account ID",
                help: "Optional channel account ID for multi-account setups.",
                control: html`
                  <input
              ...
              ${renderCheckboxRow({
                ...
                help: "Use lightweight bootstrap context for this agent job.",
...
function renderFailureAlertRows(props: CronProps, channelOptions: string[]) {
  return html`
    ${renderRow({
      ...
      help: "Control when this job sends repeated-failure alerts.",
      control: html`
        <select
    ...
    ${props.form.failureAlertMode === "custom"
      ? html`
          ${renderRow({
            label: "Alert after",
            help: "Consecutive errors before alerting.",
          ...
          ${renderRow({
            label: "Alert to",
            help: "Optional recipient override for failure alerts.",
            control: html`
              <input
