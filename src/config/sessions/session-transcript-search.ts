  ...
  for (const sessionId of sessionIds) {
    runOpenClawAgentWriteTransaction(
      (agentDatabase) => {
        ...
        const maxSeq = Number(rows[rows.length - 1]?.seq ?? -1);
...
export function searchSessionTranscripts(params: {
  ...
  const whereSession =
    sessionKeys.length > 0
      ...
      : "";
  // sqlite-allow-raw: MATCH, snippet(), and bm25() are FTS5 primitives
  // without a Kysely representation. session_key lives on the sessions row
  // so key renames never leave stale keys inside the index.
