...
function buildSnapshotFromConfig(cfg: OpenClawConfig, candidate: ShadowRouteCandidate): ModelCapabilitySnapshot {
  const providerConfig = (cfg.models?.providers as Record<string, { models?: unknown[] }> | undefined)?.[
    candidate.provider
  ];
  const modelConfig = (
    providerConfig?.models as Array<{ id: string; contextWindow?: number; maxTokens?: number; input?: string[] }>
    | undefined
...
function createDeps(cfg: OpenClawConfig, overrides: Partial<SafeRoutingServiceDeps> = {}): SafeRoutingServiceDeps {
  let clock = 1_000;
  let idCounter = 0;
  return {
    now: () => clock,
    ...
    resolveCandidates: () => resolveModelCandidateChain({ cfg, provider: "openai", model: "gpt-5.4" }),
...
describe("Phase 1 shadow routing — end-to-end acceptance", () => {
  ...
  it("never mutates real candidate resolution: resolveModelCandidateChain is identical before and after a full shadow evaluation", async () => {
    await withOpenClawTestState({ layout: "state-only", prefix: "openclaw-e2e-no-mutation-" }, async () => {
      resetTaskRegistryForTests();
      const before = resolveModelCandidateChain({ cfg: REAL_CONFIG, provider: "openai", model: "gpt-5.4" });
      ...
      const after = resolveModelCandidateChain({ cfg: REAL_CONFIG, provider: "openai", model: "gpt-5.4" });
  ...
  it("A (the real primary) is shadow-ineligible, B is shadow-eligible: audit reports theoretical=B while the real chain still starts with A", async () => {
    await withOpenClawTestState({ layout: "state-only", prefix: "openclaw-e2e-a-b-" }, async () => {
      ...
      const realChain = resolveModelCandidateChain({ cfg: REAL_CONFIG, provider: "openai", model: "gpt-5.4" });
      ...
      expect(evaluation.theoreticalChoice).toEqual({ provider: "anthropic", model: "claude-sonnet-5" });
  ...
  it("suggests CAPABLE_MODEL when every candidate is ineligible, while the shadow task itself still ends via the existing succeeded lifecycle (never blocked)", async () => {
    await withOpenClawTestState({ layout: "state-only", prefix: "openclaw-e2e-capable-model-" }, async () => {
      resetTaskRegistryForTests();
      const impossibleContract: PersistedTaskContract = {
        ...CONTRACT,
        requiredCapabilities: { ...CONTRACT.requiredCapabilities, minContextWindowTokens: 999_000_000 },
      };
      const deps = createDeps(REAL_CONFIG);
  ...
  it("rejects a provider outside the approved-providers allowlist with DATA_POLICY through the full create-evaluate-audit flow", async () => {
    await withOpenClawTestState({ layout: "state-only", prefix: "openclaw-e2e-data-policy-" }, async () => {
      resetTaskRegistryForTests();
      const deps = createDeps(REAL_CONFIG, { admissionPolicy: { approvedProviders: ["openai"] } });
  ...
  it("marks every attempt partial (not unavailable) when the evaluating process's digests have drifted from the lease's bound digests", async () => {
    await withOpenClawTestState({ layout: "state-only", prefix: "openclaw-e2e-digest-drift-" }, async () => {
      resetTaskRegistryForTests();
      const deps = createDeps(REAL_CONFIG);
      const created = createShadowObservationLease(deps, {
        contract: CONTRACT,
      ...
      const driftedDeps = createDeps(REAL_CONFIG, { configDigest: () => "sha256:a-different-process-config" });
  ...
  it("dedupes a duplicate (provider, model) candidate to exactly one persisted attempt even when the real chain contains it twice", async () => {
    await withOpenClawTestState({ layout: "state-only", prefix: "openclaw-e2e-dedup-" }, async () => {
      resetTaskRegistryForTests();
      const deps = createDeps(REAL_CONFIG, {
        resolveCandidates: () => [
          { provider: "anthropic", model: "claude-sonnet-5" },
  ...
  it("repeated audit reads never create new route attempts", async () => {
    await withOpenClawTestState({ layout: "state-only", prefix: "openclaw-e2e-repeat-read-" }, async () => {
      resetTaskRegistryForTests();
      const deps = createDeps(REAL_CONFIG);
      const created = createShadowObservationLease(deps, {
        contract: CONTRACT,
  ...
  it("rejects an out-of-session lease creation attempt end to end", async () => {
    await withOpenClawTestState({ layout: "state-only", prefix: "openclaw-e2e-forbidden-" }, async () => {
      resetTaskRegistryForTests();
      const deps = createDeps(REAL_CONFIG);
  ...
  it("correlates a real model_call_started/ended pair into the matching route attempt, completing it end to end", async () => {
    await withOpenClawTestState({ layout: "state-only", prefix: "openclaw-e2e-hook-correlate-" }, async () => {
      resetTaskRegistryForTests();
      const deps = createDeps(REAL_CONFIG);
