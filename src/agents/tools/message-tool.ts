...
type MessageToolOptions = {
  ...
  /** Stable public turn identity used only for externally visible send idempotency. */
...
export function createMessageTool(options?: MessageToolOptions): AnyAgentTool {
  ...
  return {
    ...
    execute: async (toolCallId, args, signal) => {
      ...
      const idempotencyRunId = options?.idempotencyRunId ?? options?.runId;
      if (!actionIdempotencyKey && idempotencyRunId) {
        ...
        if (!actionIdempotencyKey) {
          ...
          actionIdempotencyKey = buildMessageToolAutogeneratedIdempotencyKey({
            runId: normalizeMessageToolIdempotencyKeyPart(idempotencyRunId) ?? idempotencyRunId,
