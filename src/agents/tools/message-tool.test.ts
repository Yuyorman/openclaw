...
describe("message tool secret scoping", () => {
  ...
  it("uses the public turn identity for recovery send idempotency", async () => {
    mockSendResult();
    ...
    const input = await executeSend({
      ...
      toolOptions: {
        ...
        idempotencyRunId: "public-recovery-turn",
      },
    });

    expect(input?.params?.idempotencyKey).toMatch(
      /^public-recovery-turn:message-tool:[A-Za-z0-9_-]+:[A-Za-z0-9._:-]+$/,
    );
    expect(input?.params?.idempotencyKey).not.toContain("private-recovery-attempt");
  });
